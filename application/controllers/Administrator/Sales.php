...
class Sales extends CI_Controller
{
    ...
    {
        $data['selected_invoice'] = $this->input->post('selected_invoice');
        $this->load->view('Administrator/sales/search_returns', $data);
    }

    function sales_return_action()
    {
        $invoice = $this->input->post('invoice');
        $sm_invoice_get = $this->db->query("SELECT SaleMaster_InvoiceNo FROM tbl_salesmaster WHERE SaleMaster_SlNo='$invoice'")->result_array();
        $sm_invoice = $sm_invoice_get[0]['SaleMaster_InvoiceNo'];

        $invoice_count = $this->db->query("SELECT SaleMaster_InvoiceNo FROM tbl_salereturn WHERE SaleMaster_InvoiceNo='$sm_invoice'");

        $total_re_amount = 0;

        if ($invoice_count->num_rows() > 0) {
            ...
            $s_details = $this->db->query("SELECT SaleDetails_SlNo FROM tbl_saledetails WHERE SaleMaster_IDNo='$invoice'")->result_array();

            foreach ($s_details as $key => $value) {

                $Product_IDNo = $this->input->post('Product_IDNo_' . $value['SaleDetails_SlNo']);
                $purchase_d_id = $this->input->post('purchase_d_id_' . $value['SaleDetails_SlNo']);
                $s_d_get = $this->db->query("SELECT * FROM tbl_salereturndetails WHERE SaleReturn_IdNo='$SaleReturn_SlNo' AND SaleReturnDetailsProduct_SlNo='$Product_IDNo' AND purchase_d_id='$purchase_d_id' ")->result_array();
                $total_re_amount = $total_re_amount + $this->input->post('re_amount_' . $value['SaleDetails_SlNo']);
                $data = array(
                    'SaleReturnDetailsProduct_SlNo' => $this->input->post('Product_IDNo_' . $value['SaleDetails_SlNo']),
                    'SaleReturnDetails_ReturnQuantity' => $this->input->post('re_qty_' . $value['SaleDetails_SlNo']) + $s_d_get[0]['SaleReturnDetails_ReturnQuantity'],
                    'SaleReturnDetails_ReturnAmount' => $this->input->post('re_amount_' . $value['SaleDetails_SlNo']) + $s_d_get[0]['SaleReturnDetails_ReturnAmount'],
                    'Status' => 'a',
                    'SaleReturnDetails_brunchID' => $this->session->userdata("BRANCHid"),
                    'purchase_d_id' => $this->input->post('purchase_d_id_' . $value['SaleDetails_SlNo'])
                );
                $this->db->where('SaleReturn_IdNo', $SaleReturn_SlNo);
                $this->db->where('SaleReturnDetailsProduct_SlNo', $this->input->post('Product_IDNo_' . $value['SaleDetails_SlNo']));
                $this->db->where('purchase_d_id', $this->input->post('purchase_d_id_' . $value['SaleDetails_SlNo']));
                $this->db->update('tbl_salereturndetails', $data);
            }
        } else {

            $data = array(
                'SaleMaster_InvoiceNo' => $sm_invoice,
                'SaleReturn_ReturnDate' => date('Y-m-d'),
                'SaleReturn_ReturnAmount' => 0,
                'Status' => 'a',
                'SaleReturn_brunchId' => $this->session->userdata("BRANCHid"),
            );
            $this->db->insert('tbl_salereturn', $data);
            $return_id = $this->db->insert_id();
            $s_details = $this->db->query("SELECT SaleDetails_SlNo FROM tbl_saledetails WHERE SaleMaster_IDNo='$invoice'")->result_array();
            foreach ($s_details as $key => $value) {
                if ($this->input->post('re_qty_' . $value['SaleDetails_SlNo']) < 1) {
                    $re_amount = '0';
                } else {
                    $re_amount = $this->input->post('re_amount_' . $value['SaleDetails_SlNo']);
                }

                $total_re_amount = $total_re_amount + $re_amount;
                $data = array(
                    'SaleReturn_IdNo' => $return_id,
                    'SaleReturnDetailsProduct_SlNo' => $this->input->post('Product_IDNo_' . $value['SaleDetails_SlNo']),
                    'SaleReturnDetails_ReturnQuantity' => $this->input->post('re_qty_' . $value['SaleDetails_SlNo']),
                    'SaleReturnDetails_ReturnAmount' => $re_amount,
                    'Status' => 'a',
                    'SaleReturnDetails_brunchID' => $this->session->userdata("BRANCHid"),
                    'purchase_d_id' => $this->input->post('purchase_d_id_' . $value['SaleDetails_SlNo'])
                );
                $this->db->insert('tbl_salereturndetails', $data);
            }
        }
        $get_pre_re_amo = $this->db->query("SELECT SaleReturn_ReturnAmount FROM tbl_salereturn WHERE SaleMaster_InvoiceNo='$sm_invoice' ")->result_array();
        $pre_re_amo = $get_pre_re_amo[0]['SaleReturn_ReturnAmount'];
        $data_m = array(
            'SaleReturn_ReturnAmount' => ($pre_re_amo + $total_re_amount)
        );
        $this->db->where('SaleMaster_InvoiceNo', $sm_invoice);
        $this->db->update('tbl_salereturn', $data_m);
        echo 'success';
    }
    ...
    public function addSales()
    {
        ...
        try {
            ...
            $sales = array(
                ...
                'bank_name'                      => $data->sales->bank_name,
                ...
                'deliveryCost'                   => $data->sales->deliveryCost,
            ...
            foreach ($data->cart as $cartProduct) {
                ...
                $pdetailQn = $this->db->query("SELECT PurchaseDetails_TotalQuantity as pqty,PurchaseDetails_SlNo FROM tbl_purchasedetails WHERE Product_IDNo=?", $cartProduct->productId)->result_array();
                $pQtyT = 0;
                $newPQtyT = 0;
              ...
              select sum(sd.SaleDetails_TotalQuantity) as sold_qty FROM tbl_saledetails as sd WHERE sd.purchase_d_id = '$lot_id' AND sd.Product_IDNo = '$productId'
              )  as sold_qty,
              (
             ...
             select sum(pr.PurchaseReturnDetails_ReturnQuantity) as purchase_return_qty FROM tbl_purchasereturndetails as pr WHERE pr.purchase_d_id = '$lot_id' AND pr.PurchaseReturnDetailsProduct_SlNo='$productId'
              ...
              (
                 select sum(pud.PurchaseDetails_TotalQuantity) as purchase_qty FROM tbl_purchasedetails as pud WHERE  pud.PurchaseDetails_SlNo = '$lot_id' AND  pud.Product_IDNo='$productId'
                ) as purchase_qty,
    ...
    public function getSaleDetails()
    {
        ...
        if (isset($data->categoryId) && $data->categoryId != '') {
            ...
            $clauses .= " and pc.ProductCategory_SlNo <> '0'";
        }
    ...
    public function getSalesRecord()
    {
        ...
        foreach ($sales as $sale) {
            $sale->saleDetails = $this->db->query("
                select 
                    sd.*,sum(sd.SaleDetails_TotalQuantity) as SaleDetails_TotalQuantity,
                    p.Product_Name,
                    pc.ProductCategory_Name
                from tbl_saledetails sd
                join tbl_product p on p.Product_SlNo = sd.Product_IDNo
    ...
    function deleteSoldProduct()
    {
        $data = json_decode($this->input->raw_input_stream);
        $this->db->where('SaleMaster_IDNo', $data->salesId);
        $this->db->where('Product_IDNo', $data->ProdNo);
        $query_result = $this->db->delete('tbl_saledetails');
        if ($query_result) {
            echo 'deleted';
        } else {
            echo 'nodeleted';
    ...
    public function getSales()
    {
        ...
        if (isset($data->salesId) && $data->salesId != 0 && $data->salesId != '') {
            ...
            $saleDetails = $this->db->query("select 
                    sd.*,
                   FORMAT(sum( sd.SaleDetails_TotalQuantity ),2) as SaleDetails_TotalQuantity,
                    p.Product_Name,
                    pc.ProductCategory_Name,
                    u.Unit_Name
                from tbl_saledetails sd
                ...
                where sd.SaleMaster_IDNo = ? GROUP BY p.Product_SlNo
            ", $data->salesId)->result();
    ...
    public function updateSales()
    {
        ...
        try {
            ...
            foreach ($data->cart as $cartProduct) {
                $getSaleDQty = $this->db->query("SELECT sum(SaleDetails_TotalQuantity) as sale_d_qty  FROM tbl_saledetails  WHERE SaleMaster_IDNo=? AND Product_IDNo=? ", [$salesId, $cartProduct->productId])->result_array();
                $sale_d_qty = $getSaleDQty[0]['sale_d_qty'];

                if ($cartProduct->quantity == $sale_d_qty) {
                    continue;
                } else {
                    $this->db->where('SaleMaster_IDNo', $salesId);
                    $this->db->where('Product_IDNo', $cartProduct->productId);
                    $stmt_result = $this->db->delete('tbl_saledetails');
                    if ($stmt_result) {
...
<?php
    ...
    public function getProfitLoss()
    {
        ...
        if (isset($data->product) && $data->product != '') {
            $pClause = " and sd.Product_IDNo = '$data->product'";
        }
        ...
        foreach ($sales as $sale) {
            $sale->saleDetails = $this->db->query("
                select
                    ...
                    (pd.PurchaseDetails_Rate * SaleDetails_TotalQuantity) as purchased_amount,
                    (select sd.SaleDetails_TotalAmount - purchased_amount) as profit_loss
                from tbl_saledetails sd 
                left join tbl_purchasedetails pd on pd.PurchaseDetails_SlNo = sd.purchase_d_id
                join tbl_product p on p.Product_SlNo = sd.Product_IDNo
                where sd.SaleMaster_IDNo = ?
                $pClause
            ", $sale->SaleMaster_SlNo)->result();
        ...
        //             p.Product_Code,
        //             p.Product_Name,
        //             (sd.Purchase_Rate * SaleDetails_TotalQuantity) as purchased_amount,
        //             (select sd.SaleDetails_TotalAmount - purchased_amount) as profit_loss,
        //             sum(sd.SaleDetails_TotalAmount) as SaleDetails_TotalAmount
        //         from tbl_saledetails sd 
        //         join tbl_product p on p.Product_SlNo = sd.Product_IDNo
        //         where sd.SaleMaster_IDNo = ?
        //         GROUP BY sd.Product_IDNo
