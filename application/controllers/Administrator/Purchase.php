...
class Purchase extends CI_Controller
{
    ...
    public function addPurchase()
    {
        ...
        try {
            ...
            $purchase = array(
                ...
                'payment_method'                => $data->purchase->payment_method,
                'bank_name'                     => $data->purchase->bank_name,
    ...
    public function updatePurchase()
    {
        ...
        try {
            ...
            foreach ($data->cartProducts as $key => $product) {


                if ($product->PurchaseDetails_SlNo > 0) {

                    // $availableStock = $this->db->query("SELECT * FROM tbl_purchasedetails WHERE PurchaseDetails_SlNo='$PurchaseDetails_SlNo'")->num_rows();
                    ...
                    $this->db->where('PurchaseDetails_SlNo', $product->PurchaseDetails_SlNo);
                    $result = $this->db->update('tbl_purchasedetails', $data);
                } else {
                    $data = array(
                        'PurchaseMaster_IDNo' => $purchaseId,
                        'Product_IDNo' => $product->productId,
                ...
                //     );
                //      $this->db->where('PurchaseDetails_SlNo', $product->PurchaseDetails_SlNo);
                //      $this->db->update('tbl_purchasedetails', $data);
                // }else{
                //       $data = array(
                //         'PurchaseDetails_TotalQuantity'=>$product->quantity,
                //         'PurchaseDetails_TotalAmount'=>$product->total,
                ...
                // $this->db->where('PurchaseMaster_IDNo', $purchaseId);
                // $this->db->where('Product_IDNo', $product->productId);
                // $this->db->update('tbl_purchasedetails', $purchaseDetails);
