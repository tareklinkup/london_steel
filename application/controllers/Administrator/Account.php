...
class Account extends CI_Controller
{
    ...
    function getOtherIncomeExpense()
    {
        ...
        if (isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != '') {
            ...
            $commissionClause_Purchase = " and pm.PurchaseMaster_OrderDate between '$data->dateFrom' and '$data->dateTo'";
            $commissionClause = " and ct.Tr_date between '$data->dateFrom' and '$data->dateTo'";
        }
        ...
        $result = $this->db->query("
            ...
            (
                ...
                and pm.status = 'a'
                $commissionClause_Purchase
            ) as purchaseTransportCost,

            (
                select ifnull(sum(ct.Commission), 0)
                from tbl_cashtransaction ct
        ...
        $transactions = $this->db->query("
            select * from(
                ...
                select 
                    ...
                    concat('Purchase payment by Bank - ', s.Supplier_Name, ' (', s.Supplier_Code, ')') as description, 
                    pm.bank_name,
                    pm.PurchaseMaster_OrderDate as transaction_date,
                    'withdraw' as transaction_type,
                    pm.PurchaseMaster_PaidAmount as amount,
                    'Purcase payment by Bank' as note,
                    ac.account_name,
                    ac.account_number,
                    ac.bank_name,
                    ac.branch_name
                ...
                select 
                    ...
                    concat('Sale received by Bank - ', c.Customer_Name, ' (', c.Customer_Code, ')') as description, 
                    sm.bank_name,
                    sm.SaleMaster_SaleDate as transaction_date,
                    'deposit' as transaction_type,
                    sm.SaleMaster_PaidAmount as amount,
                    'Sale receive by Bank' as note,
                    ac.account_name,
                    ac.account_number,
                    ac.bank_name,
                    ac.branch_name
                from tbl_salesmaster sm
                join tbl_bank_accounts ac on ac.account_id = sm.bank_name
                left join tbl_customer c on c.Customer_SlNo = sm.SalseCustomer_IDNo
                where sm.bank_name <> 0
                and sm.Status = 'a'
