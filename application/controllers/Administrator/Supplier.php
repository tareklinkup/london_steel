...
class Supplier extends CI_Controller
{
    ...
    public function getSupplierDue()
    {
        ...
        $supplierDues = $this->db->query("
            ...
            s.Supplier_Mobile,
            s.Supplier_Address,
            (select (ifnull(sum(pm.PurchaseMaster_TotalAmount - pm.commission), 0.00) + ifnull(s.previous_due, 0.00)) from tbl_purchasemaster pm
    ...
    public function getSupplierLedger()
    {
        ...
        $payments = $this->db->query("
            ...
            select
                ...
                concat('Purchase ', pm.PurchaseMaster_InvoiceNo, ' (Commission)') as description,
                0.00 as bill,
                0.00 as paid,
                0.00 as due,
                0.00 as returned,
                0.00 as cash_received,
                pm.commission as purchase_commission,
                0.00 as balance
            from tbl_purchasemaster pm
            where pm.status = 'a'
            and pm.commission <> 0
        ...
        foreach ($payments as $key => $payment) {
            ...
            $payment->balance = ($lastBalance + $payment->bill + $payment->cash_received) - ($payment->paid + $payment->returned + $payment->purchase_commission);
        }
