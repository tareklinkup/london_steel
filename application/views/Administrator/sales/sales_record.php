...
<div id="salesRecord">
	...
	<div class="row" v-bind:style="{display:(selectedCategory==null&&searchType=='category')?'block':'none'}">
		<template v-for="category in  categories">
			<table class="record-table " style="width: 48%;margin: 5px;
    border-collapse: collapse;
    float: left;overflow: hidden;" v-bind:style="{display:sales>''?'block':'none'}">
				<thead>
					<tr>
						<th>Invoice No.</th>
						<th>Date</th>
				...
				<tbody>
					<template v-for="sale in sales">
						<template v-if="sale.ProductCategory_Name==category.ProductCategory_Name">
							<tr>
								<td>{{ sale.SaleMaster_InvoiceNo }}</td>
								<td>{{ sale.SaleMaster_SaleDate }}</td>
								<td>{{ sale.Customer_Name }}</td>
								<td>{{ sale.ProductCategory_Name }}</td>
								<td>{{ sale.SaleDetails_Rate }}</td>
								<td>{{ sale.SaleDetails_TotalQuantity }}</td>
	...
	<div class="row" style="margin-top:15px;display:none;" v-bind:style="{display: sales.length > 0 ? '' : 'none'}">
		...
		<div class="col-md-12">
			<div class="table-responsive" id="reportContent">
				<table class="record-table" v-if="(searchType == '' || searchType == 'user' || searchType == 'customer' || searchType == 'employee') && recordType == 'with_details'" style="display:none" v-bind:style="{display: (searchType == '' || searchType == 'user' || searchType == 'customer' || searchType == 'employee') && recordType == 'with_details' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Invoice No.</th>
							<th>Date</th>
				...
				<table class="record-table" v-if="(searchType == '' || searchType == 'user' || searchType == 'customer' || searchType == 'employee') && recordType == 'without_details'" style="display:none" v-bind:style="{display: (searchType == '' || searchType == 'user' || searchType == 'customer' || searchType == 'employee') && recordType == 'without_details' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Invoice No.</th>
							<th>Date</th>
					...
					<tbody>
						<tr v-for="sale in sales">
							...
							<td style="text-align:right;">{{ sale.deliveryCost }}</td>
					...
					<tfoot>
						<tr style="font-weight:bold;">
							...
							<td style="text-align:right;">{{ sales.reduce((prev, curr)=>{return prev + parseFloat(curr.deliveryCost)}, 0) }}</td>
				...
				<table class="record-table" v-if="(searchType == 'category' && searchType!=null) || searchType == 'quantity'" v-bind:style="{display: (searchType == 'category' && selectedCategory!=null) || searchType == 'quantity' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Invoice No.</th>
							<th>Date</th>
