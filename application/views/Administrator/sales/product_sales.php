				...
				<div class="widget-main">
					...
					<div class="row">
						...
						<div class="col-sm-2">
							...
							<input type="password" ref="productPurchaseRate" v-model="selectedProduct.purchaseAva" v-on:mousedown="toggleProductPurchaseRate" v-on:mouseup="toggleProductPurchaseRate" readonly title="Purchase rate (click & hold)" style="font-size:12px;width:100%;text-align: center;">
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					...
					<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
						<tr v-for="(product, sl) in cart">
							...
							<td>{{ product.total }}</td>
							<td><a href="" v-on:click.prevent="removeFromCart(sl, product.productId,product.quantity)"><i class="fa fa-trash"></i></a></td>
						</tr>
							...
							<div class="table-responsive">
								<table style="color:#000;margin-bottom: 0px;border-collapse: collapse;">
									...
									<tr>
										<td>
											<div class="form-group">
												<label class="col-sm-12 control-label no-padding-right">Delivery Cost</label>
												<div class="col-sm-12">
													<input type="number" class="form-control" v-model="sales.deliveryCost" v-on:input="calculateTotal" />
												</div>
											</div>
										</td>
									</tr>
									...
									<tr>
										<td>
											<div class="form-group">
												...
												<div class="col-xs-12">
													<select v-model="sales.payment_type" id="" class="form-control" style="border-radius: 4px;padding:0px 4px;">
	...
	new Vue({
		...
		methods: {
			...
			async productOnChange() {
				...
				await axios.post('/get_avarage_purchase_rate', {
					productId: this.selectedProduct.Product_SlNo
				}).then(res => {
					this.selectedProduct.purchaseAva = res.data;
				});

				// this.selectedProduct.Product_Purchase_Rate = '49';
			...
			addToCart() {
				let product = {
					...
					purchaseRate: this.selectedProduct.purchaseAva
				}
				...
				if (cartInd > -1) {
					let obj = {
						...
						categoryName: this.selectedProduct.ProductCategory_Name,
						name: this.selectedProduct.Product_Name,
						salesRate: this.cart[cartInd].salesRate,
						vat: this.cart[cartInd].vat,
						quantity: parseFloat(this.cart[cartInd].quantity) + parseFloat(this.selectedProduct.quantity),
						total: (parseFloat(this.cart[cartInd].quantity) + parseFloat(this.selectedProduct.quantity)) * this.cart[cartInd].salesRate,
						purchaseRate: this.cart[cartInd].purchaseRate
					}
					this.cart[cartInd] = obj;
				} else {
			...
			removeFromCart(ind, ProdNo, cartQty) {
				if (this.sales.salesId != 0) {
					axios.post('/delete_sold_product', {
						ProdNo: ProdNo,
			...
			calculateTotal() {
				...
				this.sales.total = ((parseFloat(this.sales.subTotal) + parseFloat(this.sales.vat) + parseFloat(this.sales.transportCost)) - (parseFloat(this.sales.discount) + parseFloat(this.sales.deliveryCost))).toFixed(2);
			...
			saveSales() {
				...
				if (this.sales.payment_type == 'Bank' && this.sales.bank_name == '') {
			...
			getSales() {
				axios.post('/get_sales', {
					...
					this.sales.deliveryCost = sales.deliveryCost;
					...
					this.sales.bank_name = sales.bank_name;
