...
<div id="purchaseRecord">
	...
	<div class="row" v-bind:style="{display:selectedCategory==null?'block':'none'}">
		<template v-for=" category in  categories ">

			<table class="record-table " style="width: 48%;margin: 5px;
    border-collapse: collapse;
    float: left;overflow: hidden;" v-bind:style="{display:purchases>''?'block':'none'}">
				<thead>
					<tr>
						<th>Invoice No.</th>
						<th>Date</th>
				...
				<tbody>
					<template v-for="purchase in purchases">
						<template v-if="purchase.ProductCategory_Name==category.ProductCategory_Name">

							<tr>
								<td>{{ purchase.PurchaseMaster_InvoiceNo }}</td>
								<td>{{ purchase.PurchaseMaster_OrderDate }}</td>
								<td>{{ purchase.Supplier_Name }}</td>
								<td>{{ purchase.ProductCategory_Name }}</td>
								<td>{{ purchase.PurchaseDetails_Rate }}</td>
								<td>{{ purchase.PurchaseDetails_TotalQuantity }}</td>
	...
	<div class="row" style="margin-top:15px;display:none;" v-bind:style="{display: purchases.length > 0 ? '' : 'none',display:( searchType  == 'category' && selectedCategory==null)?'':'block'}">
		<div class="col-md-12" style="margin-bottom: 10px;">
			<a href="" @click.prevent="print"><i class="fa fa-print"></i> Print</a>
		</div>
		<div class="col-md-12">
			<div class="table-responsive" id="reportContent">
				<table class="record-table" v-if="(searchType == '' || searchType == 'user') && recordType == 'with_details'" style="display:none" v-bind:style="{display: (searchType == '' || searchType == 'user') && recordType == 'with_details' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Invoice No.</th>
							<th>Date</th>
					...
					<tbody>
						<template v-for="purchase in purchases">
							...
							<tr>
								...
								<td>{{ purchase.PurchaseMaster_OrderDate }}</td>
								<td>{{ purchase.Supplier_Name }}</td>
								<td>{{ (Object.keys(purchase.purchaseDetails).length === 0) ?'': purchase.purchaseDetails[0].Product_Name}}</td>
								<td style="text-align:right;">{{ (Object.keys(purchase.purchaseDetails).length === 0) ?'': purchase.purchaseDetails[0].PurchaseDetails_Rate

								  }}</td>
								<td style="text-align:center;">{{

								(Object.keys(purchase.purchaseDetails).length === 0) ?'': purchase.purchaseDetails[0].PurchaseDetails_TotalQuantity }}</td>
								<td style="text-align:right;">{{

									(Object.keys(purchase.purchaseDetails).length === 0) ?'': purchase.purchaseDetails[0].PurchaseDetails_TotalAmount
				...
				<table class="record-table" v-if="(searchType == ''  || searchType == 'user') && recordType == 'without_details'" style="display:none" v-bind:style="{display: (searchType == '' || searchType == 'user') && recordType == 'without_details' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Invoice No.</th>
							<th>Date</th>
					...
					<tbody>
						<tr v-for="purchase in purchases">
							...
							<td style="text-align:right;">{{ purchase.commission }}</td>
					...
					<tfoot>
						<tr style="font-weight:bold;">
							...
							<td style="text-align:right;">{{ purchases.reduce((prev, curr)=>{return prev + parseFloat(curr.commission)}, 0) }}</td>
				...
				<table class="record-table" v-if="searchType == 'category' || searchType == 'quantity'" style="display:none;" v-bind:style="{display: (searchType == 'category' && selectedCategory!=null) || searchType == 'quantity' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Invoice No.</th>
							<th>Date</th>
