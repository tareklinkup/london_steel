									...
									<tr>
										<td>
											<div class="form-group">
												...
												<div class="col-sm-12">
													<input type="number" id="commission" class="form-control" v-model="purchase.commission" v-on:input="calculateTotal" />
												</div>
											</div>
										</td>
									</tr>
...
<script>
	...
	new Vue({
		...
		methods: {
			...
			async addToCart() {
				let long = false;
				await axios.post('/check_original_sold_qty', {
			...
			removeFromCart(ind, productId) {
				if (this.purchase.purchaseId != 0) {
					axios.post('/delete_previous_purchase', {
						productId: productId,
						purchaseId: this.purchase.purchaseId
					}).then(res => {
						if (res.data === 'deleted') {
			...
			calculateTotal() {
				...
				if (this.selectedSupplier.Supplier_Type == 'G') {
					...
					this.purchase.due = (parseFloat(this.purchase.total) - (parseFloat(this.purchase.paid) + parseFloat(this.purchase.commission))).toFixed(2);
				}
			},
			savePurchase() {
				if (this.selectedSupplier.Supplier_SlNo == null) {
			...
			getPurchase() {
				axios.post('/get_purchases', {
					...
					this.purchase.payment_method = purchase.payment_method;
					this.purchase.bank_name = purchase.bank_name;
