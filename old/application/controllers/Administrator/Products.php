...
class Products extends CI_Controller {
    ...
    public function getProductStock(){
        ...
        $stock = $this->db->query("SELECT pd.Product_SlNo,
        (
          select sum(sd.SaleDetails_TotalQuantity) as sold_qty FROM tbl_saledetails as sd WHERE   pd.Product_SlNo = sd.Product_IDNo
        )  as sold_qty,

        (
          select sum(sdr.SaleReturnDetails_ReturnQuantity) as sold_qty_return FROM tbl_salereturndetails as sdr WHERE  pd.Product_SlNo = sdr.SaleReturnDetailsProduct_SlNo
        ) as sold_qty_return,

        (
         select sum(pud.PurchaseDetails_TotalQuantity) as purchase_qty FROM tbl_purchasedetails as pud WHERE  pd.Product_SlNo = pud.Product_IDNo
        ) as purchase_qty,

        (
         select sum(pr.PurchaseReturnDetails_ReturnQuantity) as purchase_return_qty FROM tbl_purchasereturndetails as pr WHERE  pd.Product_SlNo = pr.PurchaseReturnDetailsProduct_SlNo
        ...
        WHERE pd.Product_SlNo = ? AND pd.Product_branchid = ? ",[$inputs->productId, $this->session->userdata("BRANCHid")])->result_array();

        echo empty($stock[0]['current_quantity']) ? '0':$stock[0]['current_quantity'];
    }
    ...
    function lot_stock(){
        ...
        $data['current_stock_obj'] = $this->db->query("SELECT pd.*,p.Product_Code,p.Product_Name,pc.ProductCategory_Name,
              (
              select sum(sd.SaleDetails_TotalQuantity) as sold_qty FROM tbl_saledetails as sd WHERE pd.PurchaseDetails_SlNo = sd.purchase_d_id AND pd.Product_IDNo = sd.Product_IDNo
              )  as sold_qty,
              (
               select sum(sdr.SaleReturnDetails_ReturnQuantity) as sold_qty_return FROM tbl_salereturndetails as sdr WHERE pd.PurchaseDetails_SlNo = sdr.purchase_d_id AND pd.Product_IDNo = sdr.SaleReturnDetailsProduct_SlNo
              ) as sold_qty_return,
              (
             select sum(pr.PurchaseReturnDetails_ReturnQuantity) as purchase_return_qty FROM tbl_purchasereturndetails as pr WHERE pd.PurchaseDetails_SlNo = pr.purchase_d_id AND pd.Product_IDNo = pr.PurchaseReturnDetailsProduct_SlNo
              ) as purchase_return_qty,
              (
